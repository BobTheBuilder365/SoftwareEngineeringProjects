...
public class WebValidator_Controller {
	...
	// Validations check für die 2 Textfelder
	private boolean webAddressValid = false;
	private boolean portValid = false;

	protected WebValidator_Controller(WebValidator_Model model, WebValidator_View view) {
		this.model = model;
		this.view = view;
	...
	private void enableDisableButton() {
		...
		// "Negative Logik" -> setzte Button auf disabled wenn Werte NICHT gültig sind
		view.btnConnect.setDisable(!valid); 
	}

	protected boolean isValidWebAddress(String newValue) {
		boolean valid = false;
		// Alternativ zu umständlicher Prüfung im Email Validator
		// Mit negativer Zahl als zweiten Parameter werden leere Strings am Ende behalten
